...
var didIo__namespace = /*#__PURE__*/_interopNamespaceDefault(didIo);
...
class DidWebDriver {
   ...
   */
  constructor({
    fetchOptions = constants.defaultFetchOptions,
    allowList = []
  } = {}) {
    ...
    this.didKeyDriver = didMethodKey.driver();
  }
  ...
  async fromKeyPair({url, verificationKeyPair, keyAgreementKeyPair} = {}) {
    assertions.assertDomain({allowList: this.allowList, url});
    ...
    const {did} = helpers.httpsUrlToDidUrl(url);
    didDocument.id = did;
  ...
  async get({did, url, fetchOptions = {}} = {}) {
    ...
    const {baseUrl, fragment} = helpers.didUrlToHttpsUrl(did);
    // ensures the domain is allowed
    assertions.assertDomain({allowList: this.allowList, url: baseUrl});
    // overwrite global options with request specific options
    const requestOptions = {...this.fetchOptions, ...fetchOptions};
    const {data} = await httpClient.httpClient.get(baseUrl, requestOptions);
    ...
    if(fragment) {
      ...
      return helpers.getNode({didDocument: data, id});
    }
...
exports.DidWebDriver = DidWebDriver;
